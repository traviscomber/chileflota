...
import { AlertCircle, LayoutDashboard, TrendingUp, Truck } from "lucide-react"
import { createClient } from "@/lib/supabase/server"
import { redirect } from "next/navigation"

const navItems = [
  { href: "/mandante", label: "Inicio", icon: LayoutDashboard },
...
export default async function MandanteLayout({ children }: { children: React.ReactNode }) {
    ...
    <div className="min-h-screen bg-[var(--cf-canvas)] text-[var(--cf-text)] md:flex">
      <aside className="border-b border-[var(--cf-border)] bg-[var(--cf-sidebar)] md:fixed md:inset-y-0 md:left-0 md:w-56 md:border-b-0 md:border-r">
        <div className="border-b border-[var(--cf-border)] px-5 py-4">
          ...
          <p className="mt-1 text-xs text-[var(--cf-text-muted)]">Mandante</p>
        </div>
