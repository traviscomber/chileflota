...
export function DocumentManagerHub({ stats: initialStats }: DocumentManagerHubProps) {
  ...
  const [isCanonicalReady, setIsCanonicalReady] = useState(false)
    ...
    <div className="mx-auto max-w-7xl space-y-5">
      <header className="flex flex-col gap-4 border-b border-[#303238] pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0 max-w-3xl">
          ...
          <p className="mt-2 text-sm leading-6 text-[#A9ADB3]">
            {isCanonicalReady
              ...
              : 'Sincronizando tu cartera con la fuente canónica de documentos...'}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant="outline" className="h-8 rounded-[5px] border-[#303238] px-2.5 text-xs font-normal text-[#C6C8CC]">
            ...
            {isCanonicalReady ? `${totalChileFlota.toLocaleString('es-CL')} procesados ChileFlota` : 'Sincronizando...'}
          </Badge>
          <Button
            variant="outline"
            size="sm"
      ...
      <div className="grid grid-cols-2 gap-px overflow-hidden rounded-[5px] bg-[#303238] md:grid-cols-5">
        ...
        <Link href="/dashboard/company/documentos/aprobados" className="contents">
          <MetricCard label="Aprobados" value={isCanonicalReady ? totalAprobados : null} icon={CheckCircle} tone="success" />
        </Link>
        <Link href="/dashboard/company/documentos/rechazados" className="contents">
          <MetricCard label="Rechazados" value={isCanonicalReady ? totalRechazados : null} icon={XCircle} tone="danger" />
        </Link>
      </div>
      ...
      <p className="text-xs leading-5 text-[#777C84]">
        {isCanonicalReady
          ...
          : 'Los estados operacionales se muestran sólo después de resolver la cartera autenticada.'}
      </p>
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex flex-col items-end gap-1 text-right">
                    {module.processed !== null ? (
                      <>
                        <span className="text-xs tabular-nums text-[#C6C8CC]">{isCanonicalReady ? `${module.processed.toLocaleString('es-CL')} gestionados` : '—'}</span>
                        <span className="text-[11px] tabular-nums text-[#777C84]">{isCanonicalReady ? `${module.current.toLocaleString('es-CL')} actuales` : 'Sincronizando'}</span>
                      </>
                    ) : (
                      <span className="text-xs tabular-nums text-[#C6C8CC]">{isCanonicalReady ? `${module.current.toLocaleString('es-CL')} asignadas` : '—'}</span>
                    )}
                  </div>
                </div>
                  ...
                  {module.statItems.map((stat) => {
                    ...
                    return (
                      <div key={stat.label} className="flex min-h-8 items-center justify-between gap-3 text-xs">
                        ...
                        <span className={`font-medium tabular-nums ${stat.color}`}>{isCanonicalReady ? stat.value.toLocaleString('es-CL') : '—'}</span>
                      </div>
                    )
                  })}
                </div>
